  ...
  case WM_INIT_DIALOG:
    ...
    { 
      if(VNC_IsRunning() == 1)
      {
        ...
        TEXT_SetText(hItem, ">>>");   
        hItem = WM_GetDialogItem(pMsg->hWin, ID_FILENAME);
        TEXT_SetText(hItem, "VNC Server Running");
      }      
    ...
    case ID_RECORD_REC_CANCEL_PLAY: 
      ...
      case WM_NOTIFICATION_RELEASED:
        ...
        if( RecorderMode == RECORDER_MODE_REC_IDLE) 
        {
          ...
          AUDIO_RECORDER_StartRec(BSP_AUDIO_FREQUENCY_16K);
        ...
        else if( RecorderMode == RECORDER_MODE_PLAY_IDLE) 
        {
          ...
          AUDIO_RECORDER_Play(BSP_AUDIO_FREQUENCY_16K);
...
static void Startup(WM_HWIN hWin, uint16_t xpos, uint16_t ypos)
{
  ...
  if (VNC_IsRunning() == 0)
  {
    if (k_ShowMessageBox(hMainWin, "Warning", "Audio Recorder and VNC server share some") == 0)
    {  
