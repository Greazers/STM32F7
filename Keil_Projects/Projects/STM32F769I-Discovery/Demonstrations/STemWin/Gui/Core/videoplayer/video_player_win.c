...
#define ID_BUTTON_HW_SW           (GUI_ID_USER + 0x100)
#define ID_SLIDER_VOLUME          (GUI_ID_USER + 0x101)
...
#define ID_FEATURES_HW_SW           (GUI_ID_USER + 0x44)
...
/**
  * @brief  Paints HW_SW button
  * @param  hObj: button handle
  * @retval None
  */
static void _OnPaint_hw_sw(BUTTON_Handle hObj) {
  
  GUI_SetFont(&GUI_FontAvantGarde24B);
  if(toggle_hw == 0)
  {
    GUI_SetColor(GUI_STCOLOR_LIGHTBLUE);
    GUI_DrawRect(0, 0, 76, 35);
    GUI_FillRect(1, 1, 36, 34);  
    GUI_SetColor(GUI_WHITE); 
    GUI_SetBkColor(GUI_STCOLOR_LIGHTBLUE);
    GUI_DispStringAt("ON", 3, 5);
  }
  else
  {
    GUI_SetColor(GUI_STCOLOR_LIGHTBLUE);
    GUI_DrawRect(0, 0, 76, 35);
    GUI_FillRect(36, 1, 75, 34);        
    GUI_SetColor(GUI_WHITE);  
    GUI_SetBkColor(GUI_STCOLOR_LIGHTBLUE);
    GUI_DispStringAt("OFF", 39, 5);
  }  
}
...
/**
  * @brief  callback for hw_sw button
  * @param  pMsg: pointer to data structure of type WM_MESSAGE
  * @retval None
  */
static void _cbButton_hw_sw(WM_MESSAGE * pMsg) {
  switch (pMsg->MsgId) {
  case WM_PAINT:
    _OnPaint_hw_sw(pMsg->hWin);
    break;
  default:
    /* The original callback */
    BUTTON_Callback(pMsg);
...
static void _cbplaybackwin(WM_MESSAGE * pMsg) 
{
  ...
  case WM_CREATE:
    ...
    hItem = BUTTON_CreateEx(710, 430, 80, 38, pMsg->hWin, WM_CF_SHOW, 0, ID_BUTTON_HW_SW);
    WM_SetCallback(hItem, _cbButton_hw_sw); 
    ...
    hItem = TEXT_CreateEx(200, 40, 350, 50, pMsg->hWin, WM_CF_SHOW, TEXT_CF_HCENTER, ID_VIDEO_FORMAT, "Format : AVI (MJPEG/PCM)");
    TEXT_SetFont(hItem, &GUI_FontAvantGarde20B);
    TEXT_SetTextColor(hItem, GUI_WHITE);
    
    hItem = TEXT_CreateEx(695, 220, 200, 40, pMsg->hWin, WM_CF_SHOW, TEXT_CF_LEFT, ID_FPS, "Rate : -- fps");
    TEXT_SetFont(hItem, &GUI_FontAvantGarde20B);
    TEXT_SetTextColor(hItem, GUI_WHITE);   
    
    hItem = TEXT_CreateEx(710, 405, 85, 20, pMsg->hWin, WM_CF_SHOW, TEXT_CF_LEFT, ID_FEATURES_HW_SW, "HW JPEG");
    TEXT_SetFont(hItem, &GUI_FontAvantGarde20B);
    TEXT_SetTextColor(hItem, GUI_WHITE);

    hItem = TEXT_CreateEx(695, 245, 200, 40, pMsg->hWin, WM_CF_SHOW, TEXT_CF_LEFT, ID_FEATURES_CPU, "MCU : 0%");
    TEXT_SetFont(hItem, &GUI_FontAvantGarde20B);
    TEXT_SetTextColor(hItem, GUI_WHITE);       
  ...
  case WM_PAINT: 
    ...
    /*H/W Information */
    GUI_AA_FillRoundedRect(690, 200, 900, 285, 30);    
...
static uint8_t _StartPlay(char * filename, FIL * file, uint16_t x0, uint16_t y0) 
{
  ...
  {   
    ...
    if((Video_Info.xSize != LCD_GetXSize()) || (Video_Info.ySize != LCD_GetYSize()))
    {
      ...
      hItem = WM_GetDialogItem(hFrame, ID_WARNING_MSG);
      TEXT_SetText(hItem, "Only M-JPEG Format and 800x480 video are supported");
      WM_InvalidateWindow(hItem);
      WM_Paint(hItem);      
    ...
    hItem = WM_GetDialogItem(playbackwin, ID_SLIDER_VOLUME);
    WM_HideWindow(hItem);
    WM_InvalidateWindow(hItem);
    WM_Paint(hItem);  
    hItem = WM_GetDialogItem(playbackwin, ID_VIDEO_FORMAT);
...
static void _cbVideoWindow(WM_MESSAGE * pMsg) 
{
  ...
  {
  case WM_CREATE:
    hItem = TEXT_CreateEx(100, 220, 600, 40, pMsg->hWin, WM_CF_SHOW, 0, ID_WARNING_MSG, "");
    TEXT_SetTextAlign(hItem, GUI_TA_HCENTER);
    TEXT_SetFont(hItem, &GUI_FontAvantGarde24B);
    TEXT_SetTextColor(hItem, GUI_WHITE);   
    hItem = TEXT_CreateEx(100, 170, 600, 40, pMsg->hWin, WM_CF_SHOW, 0, ID_WARNING_ERROR, "");
    TEXT_SetTextAlign(hItem, GUI_TA_HCENTER);
    TEXT_SetFont(hItem, &GUI_FontAvantGarde24B);
    TEXT_SetTextColor(hItem, GUI_WHITE);  
