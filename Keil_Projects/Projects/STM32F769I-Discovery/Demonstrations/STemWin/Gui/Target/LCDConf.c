...
int32_t DSI_IO_ReadCmd(uint32_t Reg, uint8_t *pData, uint32_t Size)
{
  ...
  if(HAL_DSI_Read(&hdsi_disco, 0, pData, Size, DSI_DCS_SHORT_PKT_READ, Reg, pData)!= HAL_OK);
  {
    ret = -1;
  }
...
{   
  ...
  */
  ...
  {
    /* Initialize the OTM8009A LCD Display IC Driver (TechShine LCD IC Driver)
  ...
  else
  {
    ...
    */
    ...
    NT35510_Init(hdsivideo_handle.ColorCoding, NT35510_ORIENTATION_LANDSCAPE);
  }
